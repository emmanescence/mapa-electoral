...
# URL del archivo comprimido con los circuitos electorales
...
# Función para calcular los resultados y generar el mapa
def generar_mapa_y_tabla(cabecera, cargo_nombre):
    ...
    # Asegurarse de que 'geometry' sea del tipo geométrico adecuado para geopandas
    geo_merged_df = gpd.GeoDataFrame(df_merged, geometry='geometry')

    # Configurar el sistema de coordenadas (CRS) si es necesario
...
st.title('Resultados Electorales 2023 - Mapa de Circuitos')

# Selección de cabecera y cargo
...
if st.button('Mostrar Mapa y Resultados'):
    generar_mapa_y_tabla(cabecera, cargo_nombre)
