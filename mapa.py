...
# Verificar si el archivo CSV existe dentro del ZIP
if archivo_csv not in zip_file_resultados.namelist():
    st.error(f"El archivo {archivo_csv} no se encuentra en el ZIP.")
else:
...
# Verificar si el archivo GeoJSON existe dentro del ZIP
if archivo_geojson not in zip_file_circ.namelist():
    st.error(f"El archivo {archivo_geojson} no se encuentra en el ZIP.")
else:
    # Leer el archivo GeoJSON extraído
    with zip_file_circ.open(archivo_geojson) as geojson_file:
        geo_df = gpd.read_file(geojson_file)

# Mostrar las primeras filas del GeoDataFrame y del DataFrame CSV en Streamlit
st.write("Primeras filas del GeoDataFrame:")
st.write(geo_df.head())

st.write("Primeras filas del DataFrame CSV:")
...
def mostrar_mapa_simple():
    fig, ax = plt.subplots(figsize=(15, 15))
    geo_df.plot(ax=ax, edgecolor='k', color='lightblue')
    ax.set_title('Mapa Simple')
    st.pyplot(fig)

# Mostrar un mapa simple para probar
mostrar_mapa_simple()
